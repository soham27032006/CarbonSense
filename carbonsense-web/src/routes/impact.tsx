...
const equivalencyFacts: Record<string, string> = {
  ...
  flights_ny_to_la: "A NYC to LA flight emits about 900 kg CO2.",
  showers: "A 10-minute hot shower creates about 1.5 kg CO2.",
  showers_skipped: "A 10-minute hot shower creates about 1.5 kg CO2.",
  shower_minutes: "A 10-minute hot shower creates about 1.5 kg CO2.",
};
...
function ImpactPage() {
  ...
  useEffect(() => {
    if (
      ...
      toast.error("Some impact data is still syncing. Showing available data.", {
        id: "impact-load-error",
      });
    }
  }, [totalQuery.isError, equivQuery.isError, levelQuery.isError, achievementsQuery.isError]);

  const loading = totalQuery.isLoading && !total;
  const missingAnyLiveData = !total || !equiv || !level || !achv;
  const refetchAll = () => {
    totalQuery.refetch();
    equivQuery.refetch();
    levelQuery.refetch();
  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden bg-background text-foreground">
      <Ambient />
      <StickyHeader avatarName={firstName} left={<ImpactHeaderLeft />} center={<ImpactHeaderCenter />} />
      ...
      >
        {loading ? (
          <ImpactSkeleton />
        ) : missingAnyLiveData && !total && !equiv && !level && !achv ? (
          <ImpactLoadError onRetry={refetchAll} />
        ) : (
          <>
            {total ? (
              ...
              <SectionFallback
                title="Your forest is taking root"
                message="Forest data is still syncing from the backend."
                onRetry={() => totalQuery.refetch()}
              />
            )}
            <LifetimeStats
              total={total}
              level={level}
              achievements={achv}
              onRetryTotal={() => totalQuery.refetch()}
              onRetryLevel={() => levelQuery.refetch()}
              onRetryAchievements={() => achievementsQuery.refetch()}
            />
            <Equivalencies items={equiv?.items ?? []} loading={equivQuery.isLoading} onRetry={() => equivQuery.refetch()} />
            {level ? (
              <LevelProgress level={level} />
            ) : (
              <SectionFallback
                title="Level progress"
                message="Level data is still syncing from the backend."
                onRetry={() => levelQuery.refetch()}
              />
            )}
            <AchievementsGallery
              data={achv}
            ...
            {total && level ? (
              <ShareCard
                ...
                trees={getTreeCount(total.carbon_saved_kg)}
                levelName={level.current.name}
              />
            ) : (
              <SectionFallback
                title="Share your impact"
                message="Available once your level and footprint have loaded."
                onRetry={refetchAll}
              />
            )}
          </>
    ...
    >
      ...
      <div className="relative flex items-center justify-between gap-3 px-5 py-4 sm:px-6">
        <div>
          ...
          <p className="mt-1 text-xs text-emerald-100/75">{getForestMessage(treeCount)}</p>
        </div>
...
function LifetimeStats({
  ...
  total: ImpactTotal | null;
  level: LevelResp | null;
  achievements: AchievementsResp | null;
  onRetryTotal: () => void;
  onRetryLevel: () => void;
  onRetryAchievements: () => void;
}) {
  const { unitSystem } = useUnits();
  if (!total && !level && !achievements) {
    return (
      <SectionFallback
        title="Lifetime stats"
        message="Your lifetime footprint data is still syncing."
        onRetry={() => {
          onRetryTotal();
          onRetryLevel();
          onRetryAchievements();
  ...
  const achievementTotal = achievements
  ...
  return (
    <section className="mt-8">
      ...
      {loading ? (
        ...
        <SectionFallback
          title="Equivalencies"
          message="Real-world equivalents appear once your impact data is available."
          onRetry={onRetry}
        />
      )}
    </section>
...
function EquivCard({ item, index }: { item: Equivalency; index: number }) {
  ...
  const isFractional = item.value > 0 && item.value < 1;
  const unit = isFractional || rounded === 0
    ? config.unit
    : pluralize(rounded, config.unit, `${config.unit}s`);
      ...
      <div className="relative flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex flex-wrap items-baseline gap-x-1 gap-y-0.5">
            <span className="text-3xl font-bold tracking-tight tabular-nums whitespace-nowrap">
              <CountUp value={item.value} decimal={item.value > 0 && item.value < 1} formatter={formatEquivalencyValue} />
            </span>
            <span className="text-sm font-medium text-emerald-100/80 whitespace-nowrap">
              {displayValue.startsWith("<") ? config.unit : unit}
            </span>
          </div>
        </div>
      </div>
          ...
          {level.next ? (
            <>
              <span className="font-medium tabular-nums text-foreground">
                {level.xp_into_current} /{" "}
                {level.xp_into_current + level.xp_to_next}
              </span>{" "}
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[11px] text-muted-foreground tabular-nums">
                  {l.xp_required === 0 ? "Start" : `${l.xp_required.toLocaleString()} XP`}
                </p>
              </div>
              {current ? (
                <motion.span
  ...
  if (!data) {
    return (
      <section className="mt-8">
        ...
        <SectionFallback
          title=""
          message="Achievements unlock once your account activity syncs."
          onRetry={onRetry}
        />
      </section>
    );
  }
  const sourceAchievements = data.achievements;
    ...
    <section className="mt-8">
      ...
      {loading ? (
        ...
        <p className="mt-4 rounded-2xl border border-white/10 bg-white/[0.03] p-5 text-center text-sm text-muted-foreground">
          No achievements yet — complete challenges to start unlocking them.
        </p>
      ) : (
...
function ImpactLoadError({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-6 text-center">
      <p className="text-lg font-semibold">Couldn't load your live impact data.</p>
      <p className="mt-2 text-sm text-muted-foreground">
        Impact stats are only shown after the backend returns your account data.
      </p>
      <button
        type="button"
        onClick={onRetry}
  ...
  return (
    <div className="mt-4 rounded-2xl border border-dashed border-white/10 bg-white/[0.02] p-5 text-center">
      ...
      <button
        type="button"
        onClick={onRetry}
        className="mt-3 inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-medium text-foreground hover:bg-white/10"
      >
        Retry
      </button>
    </div>
