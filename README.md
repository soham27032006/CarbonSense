...
| **Testing** | Vitest suite: 12 test files, 63 passing tests, ~7.2s runtime; **53.05% line coverage** (52.74% statements, 43.57% branches) via `@vitest/coverage-v8`; shared Supabase/env/Redis/AI mocks; covers carbon service math, Plaid token round-trip, profile updates, challenge acceptance, XP/level math, team join flow, Gemini retry, rate-limit validation, impact totals (94.9% lines), and streak milestones (100% lines) | `carbonsense-api/vitest.config.ts`; `carbonsense-api/tests/` (12 files across services/ and controllers/ and middleware/); runs green via `npm test` in `carbonsense-api/`; coverage via `npm run test:coverage` |
...
Expect: **12 files, 53 tests, all passing in ~13.6s.**
